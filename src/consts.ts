  ...
  get () {
    ...
    return {
      ...
      regexes: {
        ...
        tagSpecial: new RegExp(`(?:^|[^a-zA-Z0-9-])@(${tagsNames.map(n => _.escapeRegExp(n)).join('|')})(?:(?:\\([^)]*\\))|(?![a-zA-Z0-9-]))`, 'gm'),
        tagSpecialNormal: new RegExp(`(?:^|[^a-zA-Z0-9-])(?:${tagsNames.map(n => `(@${_.escapeRegExp(n)}(?:(?:\\([^)]*\\))|(?![a-zA-Z0-9-])))`).join('|')}|(@[^\\s*~(]+(?::\/\/[^\\s*~(:]+)?(?:(?:\\([^)]*\\))|(?![a-zA-Z]))))`, 'gm'),
